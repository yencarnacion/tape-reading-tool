# ADR Extension from Running RTH Low or High
...
```text
From low = (Current_t / Running_RTH_Low_t - 1) / ADR_N
From high = (Running_RTH_High_t / Current_t - 1) / ADR_N
```

The direction mode defaults to `AUTO`. AUTO selects `FROM LOW` when the current price is at or above the RTH open and `FROM HIGH` below the RTH open. The trader can override it with `FROM LOW` or `FROM HIGH`. Both directional readings remain visible in every mode; AUTO changes only the emphasized headline value and does not classify a trend or predict a reversal.
...
Nominal RTH is 09:30:00 inclusive through 16:00:00 exclusive in `America/New_York`, using timezone-aware conversion across daylight-saving changes. The current price and running high/low use the same chart-eligible trade population as the price chart. Bid, ask, midpoint, rejected/unreported prints, premarket prices, and final daily extremes loaded in advance are excluded.
...
The browser does not infer the high or low from its bounded tape array. A core-owned session-context capability returns symbol, session, as-of time, OHLC/high and low times/latest price, eligible count, source/provider/mode, and `completeFromRTHOpen`.

For live IBKR, the existing connection supplies bounded historical bars; no second connection is created. Massive and replay use locally recorded/downloaded coverage when available. Demo supplies deterministic prior bars and session context. If an application starts or changes symbols after 09:30 and the core cannot prove coverage back to the open, the UI shows `RTH RANGE INCOMPLETE`.
...
These guarantees are exercised against a real replay feed, not a simulated position. `internal/server/panel_replay_test.go` steps a prepared render to exact instants and asserts that a session low formed later in the session is invisible before the position reaches it, that a request naming an instant past the position clamps rather than revealing it, that the baseline is selected from the replay session date even when the browser asks with a wall-clock date, and that the same position twice produces the same context. `scripts/replay-panel-check.mjs` drives the same guarantees through the browser against a generated recording of two symbols over two consecutive sessions: that the running low the panel reports is one the browser's own tape never received because the seek restarted it later in the session, and that a backward seek into the previous session and a ticker change mid-replay each reload the panel rather than leaving the previous answer on screen.
...
- VWAP-based automatic direction, automatic time-of-day panel switching, signals, alerts, backtests, and order entry are outside scope.
