...
The browser's own clock is an extrapolation between deliveries, and a seek invalidates it: a replay seek publishes an empty same-symbol snapshot, so the retained trades belong to the position the seek left and the receipt clock is not resynchronised from them. Every snapshot therefore carries the authoritative instant — the replay position in replay and render modes — and the browser adopts it at each generation boundary. Without that the panel would ask the core for the instant it had just moved away from, and after a cross-session seek for the wrong session entirely, which the core would then answer for its own session and the panel would reject as stale.

These guarantees are exercised against a real replay feed, not a simulated position: `internal/server/panel_replay_test.go` steps a prepared render to exact instants and asserts that a session low formed later in the session is invisible before the position reaches it, that a request naming an instant past the position clamps rather than revealing it, that the baseline is selected from the replay session date even when the browser asks with a wall-clock date, and that the same position twice produces the same context.
