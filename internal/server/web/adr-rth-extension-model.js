...
export function seedRTHContext(payload, { symbol, sessionDateET } = {}) {
  ...
  const low = Number(payload.low), last = Number(payload.last);
  if (![low, last].every((value) => Number.isFinite(value) && value > 0)) return { status: 'building', symbol, sessionDateET };
  return {
    status: payload.status === 'closed' ? 'closed' : 'ready', symbol, sessionDateET,
    low, lowTimeUS: Number(payload.lowTimeUS) || 0, last, lastTimeUS: Number(payload.lastTimeUS) || 0,
...
export function calculateExtension(adrResult, context) {
  ...
  const low = Number(context.low), last = Number(context.last);
  if (![low, last].every((value) => Number.isFinite(value) && value > 0)) return { status: 'building' };
  const percent = last / low - 1;
  return { status: context.status === 'closed' ? 'closed' : 'ready', percent, extension: percent / adrResult.adr };
}
